                ...
                {[
                  {
                    ...
                    value: db.patients.length.toString(),
                    trend: "+1",
                    trendDirection: "up"
                  },
                  {
                    label: "Rendez-vous",
                    value: db.appointments.filter(a => a.status === 'Confirmé').length.toString(),
                    trend: "Aujourd'hui",
                    trendDirection: "neutral"
                  },
                  {
                    label: "Revenus Total",
                    value: `${db.payments.reduce((acc, p) => acc + p.amount, 0).toLocaleString('fr-DZ')} DA`,
                    trend: db.payments.length > 0 ? "Actif" : "0",
                    trendDirection: "up"
                  },
                  {
                    label: "Objectif",
                    value: `${Math.min(100, Math.round((db.payments.reduce((acc, p) => acc + p.amount, 0) / (db.monthlyGoal || 1)) * 100))}%`,
                    trend: "Mensuel",
                    trendDirection: "neutral"
                  }
                ].map((kpi, i) => (
              ...
              <div className="grid lg:grid-cols-2 gap-8">
                <div className="bg-white p-8 rounded-[2.5rem] shadow-sm shadow-slate-200/50 border border-slate-50">
                  ...
                  {db.patients.length === 0 ? (
                    <p className="text-slate-400 text-sm italic">Aucun patient enregistré.</p>
                  ) : (
                    <div className="space-y-4">
                ...
                <div className="bg-white p-8 rounded-[2.5rem] shadow-sm shadow-slate-200/50 border border-slate-50">
                  ...
                  {db.appointments.length === 0 ? (
                    <p className="text-slate-400 text-sm italic">Aucun rendez-vous planifié.</p>
                  ) : (
                    <div className="space-y-4">
                      {db.appointments.filter(a => a.status !== 'Annulé').slice(0, 5).map(a => (
                        <div key={a.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl">
                          ...
                          <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-lg">{a.time}</span>
                        </div>
                      ))}
                    </div>
                  )}
